...
# Sidebar menu
menu_option = st.sidebar.radio("Pilih Menu:", ["Overall Insight Semua Negara", "Analisis SGDs Indonesia"])

if menu_option == "Overall Insight Semua Negara":
    st.header("Negara dengan nilai SDGs terbesar di tahun 2023")
    ...
    st.write("Dapat dilihat bahwa Finlandia menempati posisi pertama dalam negara dengan skor SDGs rerata, diikuti oleh Swedia, Denmark, Jerman, Austria, Prancis, Norwegia, Czech, Polandia, dan Estonia")
    ...
    st.header("Perbandingan Rata-Rata Nilai SDGs Setiap Region")
    ...
    st.pyplot(plt.gcf())
    plt.close()
    st.write("Negara-negara yang termasuk dalam OECD (Economic Co-operation and Development) memiliki rata-rata skor SDGs lebih besar dibandingkan dengan region lainnya, sedangkan region Sub-Saharan Africa memiliki rata-rata skor paling rendah. Finlandia yang merupakan negara dengan skor SDGs tertinggi termasuk dalam OECD")
    st.divider()
    ...
    st.write("Dari heatmap, kita mendapat suatu fakta unik tentang skor-skor goal SDGs yaitu ada beberapa skor yang memiliki korelasi baik itu secara positif maupun negatif. Skor-skor yang dimaksud adalah: ")
...
# plt.close()
...
elif menu_option == "Analisis SGDs Indonesia":
    st.header("Statistik Skor SDGs Indonesia")
    ...
    st.write("Skor SDGs tertinggi yang dimiliki oleh Indonesia yaitu di bidang Climate Action, Quality Education, dan Konsumsi Produksi Bertanggung Jawab. Dari bar ini juga dapat kita lihat bahwa Indonesia harus menekankan dalam bidang Partnership for The Goals, Kehidupan di Darat, dan Industri Inovasi dan Infrastruktur")
    st.divider()
